...
Rot_Speed_Vect_ast = Ast_Rot_Axis*Rot_Speed_norm_ast; %rotationnal speed of 
%the asteroif as a vector
...
V_SC = sqrt(Mu_ast/(norm(Alt_SC))); %Velocity Vector in inertiel ref frame
V_SC_ECI = [0 V_SC 0]-cross(Rot_Speed_Vect_ast, R0_SC);
X0_SC_ECI = [R0_SC V_SC_ECI];
...
% that will compute the derivative of hte state-vector and that we will
% then integrate through ode45. The function that provides dx is called
% 'TWO_BP_Pos_Vel'

options = odeset('AbsTol',1e-9,'RelTol',1e-9);
[Tnominal,SV_Nominal] = ode45(@(t,x)TWO_BP_Pos_Vel(t,x,Mu_ast,Rot_Speed_norm_ast),tspan,X0_SC_ECI,options);
...
%will look like outputs from sensors. The first step in done in this
...
options = odeset('AbsTol',1e-9,'RelTol',1e-9);
[~,SV_Nominal_Obs] = ode45(@(t,x)TWO_BP_Pos_Vel(t,x,Mu_ast,Rot_Speed_norm_ast),tspan_sec,X0_SC_ECI,options);
 
%% Creation of the measurements

%The aim of this section is to take the datas extracted in hte previous
...
Qi = zeros(6,6,nb_Points); %Covariance matrix associated sith the measuring process 
Phi = zeros(6,6,nb_Points); %STM
SV_Obs = zeros (nb_Points, 6); %Observation matrix
 
%Definition of the error in the covariance matix
SigmaPos = 10;        %[m] the 3-sigman error of the position from the measurement
qqx = (SigmaPos/3)^2; %Variance in position for the X axis
qqy = (SigmaPos/3)^2; %Variance in position for the Y axis
qqz = (SigmaPos/3)^2; %Variance in position for the Z axis

%Creation of the Covariance matrix related with the observations
Q_0 = diag([qqx,qqy,qqz]); %Cov Matrix 
cQ = diag(Q_0); 
...
%The aime of this section is to provide the s/c a first guess of the
%trajectory he's following so that he will be able to start the firt loop
%of the NLS process. To do so, we will corrupt the nomina trajectory to
%obtain something close enough that coud be used as a first guess

 SigmaVel_FirstGuess = 0.1*V_SC; % 3-sigma error for the velocity
 SigmaPos_FirstGuess = 50;       % 3-sigma error for the position
 qqPos = (SigmaPos_FirstGuess/3)^2;  %Variance for the psotion
...
SV_FirstGuess = [Pos_FirstGuess, Vel_FirstGuess];%Initial state vector of 
%First guess
 
Phi0 = eye(6); %initialisation of the STM
reshape_Phi0 = reshape(Phi0, 1, 36); %Reshape Phi0 as a row vector for simplicity for all the computation
...
%reference trajectory. The X_FirstGuess vector will contain every potision
...
%plot the trajector of the first guess (black line on the plot)
[~,X_FirstGuess] = ode45(@(t,x)TWO_BP_STM(t,x,Mu_ast,Rot_Speed_norm_ast),tspan_sec,X0_FirstGuess,options);
[~,X_FirstGuess_Traj] = ode45(@(t,x)TWO_BP_Pos_Vel(t,x,Mu_ast,Rot_Speed_norm_ast),Tnominal,SV_FirstGuess,options);
...
%each loop of the NLS until a ceratin condition on the residuals is
%achiived
 
%First we do a loop of the NLZ to obtain our first correction
[x0new, Delx_Vec, PDelx_Mat, RRi] = NLSCameraSimple(X_FirstGuess,SV_Obs,tspan_sec,Q_0);
 
%Now we initialise the loop

N = 10;                 % Arbitrary Number of iteration to initialize the size of the matrices used
X0_update = zeros(N,6); %the updated initial state vetor that the NLSCameraSImple will output
Residual = zeros(N,1); 
Correction_applied  = zeros(N,6);
Cov_Mat_Correction_applied =zeros(6,6,N);
...
while Condition>0.01
     ...
     x0_newIter(1:6) = X0_update(j-1,:); %x0_newIter is used to propagate the...
      ...
      %we can propagate
     x0_newIter(7:42) = reshape(Phi0,1,36); %Reshaphe the STM into a row vector
    
     %Now we intergrate the x0_newIter to obtain the trajectory
     options = odeset('AbsTol',1e-6,'RelTol',1e-9);
     [T,X_corrected] = ode45(@(t,x)TWO_BP_STM(t,x,Mu_ast,Rot_Speed_norm_ast),tspan_sec,x0_newIter,options);
     ...
     [x0new, Delx_Vec, PDelx_Mat, RRi] = NLSCameraSimple(X_corrected,SV_Obs,tspan_sec,Q_0);
...
%semilogy([1:N],Residual)
%% Ploting of the different trajectory

%First the asteroid is plotted
Rmean=250;
...
for j=1:N
     x0new=X0_update(j,:);
     [~,X_refined] = ode45(@(t,x)TWO_BP_Pos_Vel(t,x,Mu_ast,Rot_Speed_norm_ast),Tnominal,x0new,options);
